...
def global_average_precision(predictions, labels, top_k=20):
    ...
    gap = AP()
    sparse_predictions, sparse_labels, num_positives = _top_k_by_class(predictions, labels, top_k)
    gap.store(_flatten(sparse_predictions), _flatten(sparse_labels), sum(num_positives))
...
class EvaluationMetrics(object):
    ...
    def store(self, predictions, labels):
        ...
        mean_hit_at_one = hit_at_one(predictions, labels)
        mean_perr = precision_at_equal_recall_rate(predictions, labels)
        mean_lrap = label_ranking_average_precision_score(labels, predictions)
        mean_precision = average_precision_score(labels, predictions, average='weighted')

        # Take the top 20 predictions.
        sparse_predictions, sparse_labels, num_positives = _top_k_by_class(predictions, labels, self.top_k)
        self.mean_ap.store(sparse_predictions, sparse_labels, num_positives)
        self.global_ap.store(_flatten(sparse_predictions), _flatten(sparse_labels), sum(num_positives))
