...
class DenseMoE(Layer):
    """Dense Mixture of experts layer. This layer implements:
    y = sum_{k=1}^K g(v_k * x) f(W_k * x)
    ...
    # Output shape
        ...
        For example, for a 2D input with shape (batch_size, input_dim),
    ...
    n_experts: int
        Number of experts (K).
    expert_activation:
        Activation function for the expert model (f).
    gating_activation:
        Activation function for the gating model (g).
    use_expert_bias: bool
        Whether to use biases in the expert model.
    use_gating_bias: bool
        Whether to use biases in the gating model.
    expert_kernel_initializer_scale: float
        Scale of Glorot uniform initialization for expert model weights.
    gating_kernel_initializer_scale: float
        Scale of Glorot uniform initialization for gating model weights.
    expert_bias_initializer: str
        Initializer for the expert biases.
    gating_bias_initializer: str
        Initializer fot the gating biases.
    expert_kernel_regularizer:
        Regularizer for the expert model weights.
    gating_kernel_regularizer:
        Regularizer for the gating model weights.
    expert_bias_regularizer:
        Regularizer for the expert model biases.
    gating_bias_regularizer:
        Regularizer for the gating model biases.
    expert_kernel_constraint:
        Constraints for the expert model weights.
    gating_kernel_constraint:
        Constraints for the gating model weights.
    expert_bias_constraint:
        Constraints for the expert model biases.
    gating_bias_constraint:
        Constraints for the gating model biases.
    ...
    def build(self, input_shape):
        ...
        super(DenseMoE, self).build(input_shape)

    def call(self, inputs, **kwargs):

        expert_outputs = tensordot(inputs, self.expert_kernel, axes=1)
        if self.use_expert_bias:
        ...
        repeat_gate = K.repeat_elements(K.expand_dims(gating_outputs, axis=1), self.units, axis=1)
        output = K.sum(expert_outputs * repeat_gate, axis=2)
